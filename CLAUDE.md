...
### 実装・設定変更を伴う作業（コード、Gemfile、package.jsonなど）
...
2. Issueに対応するブランチを作成する: `<種別>/<issue番号>-<概要>`
   ...
   - 例: `feature/12-add-inquiry-entity`
...
- バックエンド（Ruby on Rails）: `3000`
- フロントエンド（Vite + Vue）: `5173`

開発サーバーを起動する際、指定のポートが既に別プロセスに使用されていて競合する場合は、以下の対応を必ず行う。

1. 該当ポートを使用しているプロセスを特定し、停止する（例: `lsof -ti:3000 | xargs kill`）
