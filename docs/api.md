...
個別に除外している。担当者の一覧・追加（`/staffs`）は管理者（`admin: true`）のみが利用でき、ログイン済みでも
管理者でない場合は `{ "error": "権限がありません" }` と `403 Forbidden` を返す。

フロントエンド（`http://localhost:5173`）からの Cookie 付きリクエストを許可するため、
...
| 権限不足（管理者専用エンドポイントに一般担当者がアクセス） | `403` | `{ "error": "権限がありません" }` |
| リソースが存在しない | `404` | `{ "error": "リソースが見つかりません" }` |
| バリデーション失敗（`POST /staffs`） | `422` | `{ "errors": [...] }` |
...
| GET | /staffs | 管理者のみ | 担当者一覧を取得する | UC8 |
| POST | /staffs | 管理者のみ | 担当者を追加する | UC8 |
...
- レスポンス（成功時）: `200 OK`、ログインした担当者の情報（`id`, `name`, `email`, `admin`）。Cookie にセッション情報が設定される
...
### GET /staffs（担当者一覧・管理者のみ）

- レスポンス（成功時）: 担当者の配列（`id` 昇順）。各要素は `id`, `name`, `email`, `admin`
- レスポンス（失敗時）: 管理者でない場合 `403`

### POST /staffs（担当者追加・管理者のみ）

- リクエスト: `name`（氏名）, `email`（メールアドレス）, `password`（8文字以上）。`admin` は受け付けない
- 追加される担当者は必ず一般権限（`admin: false`）で作成される
- レスポンス（成功時）: `201 Created`、作成された担当者（`id`, `name`, `email`, `admin`）
- レスポンス（失敗時）:
  - 管理者でない場合 `403`
  - `email` 重複・`password` が8文字未満・未入力項目がある場合 `422` と `{ "errors": [...] }`
